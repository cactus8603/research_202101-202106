...
for year in years:
    # 讀取stock_code內容
    stock = openpyxl.load_workbook("./Result/" +str(year) + '/' + str(year) + "_刪除無分析師預測後剩餘股吧名單.xlsx")
    ...
    # 存檔
    delete_stock.save(r'./Result/' + str(year) + '/' + str(year) + '_樣本小於2.5股吧名單.xlsx')
    new_stock.save(r'./Result/' + str(year) + '/' + str(year) + '_刪除樣本小於2.5後股吧名單.xlsx')
